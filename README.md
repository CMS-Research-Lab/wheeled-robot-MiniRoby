# Prototipo Educativo de Robot Móvil MiniRoby

<p align="center">
  <img src="miniroby.png" alt="Robot MiniRoby" width="400"/>
</p>

<p align="center">
  <b>Robot móvil diferencial de bajo costo para educación e investigación</b>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Estado-En%20desarrollo-yellow">
  <img src="https://img.shields.io/badge/Plataforma-Jetson%20Nano-green">
  <img src="https://img.shields.io/badge/Microcontrolador-Arduino%20Nano-blue">
  <img src="https://img.shields.io/badge/Licencia-Académica-lightgrey">
</p>

---

## Publicación asociada

Este repositorio contiene los archivos del artículo:

**"Construction of an Educational Prototype of a Differential Wheeled Mobile Robot"**  
Autores: Celso Márquez-Sánchez, Jacobo Sandoval-Gutiérrez y Daniel Librado Martinez-Velazquez  
Revista: *Hardware* (MDPI)
Como citar:


---

## Descripción del proyecto

MiniRoby es un robot móvil diferencial, de bajo costo e impreso en 3D, diseñado para:

- Enseñanza de robótica móvil  
-  Investigación en control y seguimiento de trayectorias  
-  Desarrollo de sistemas embebidos modulares  

El sistema integra:
-  **Arduino Nano** → Control de bajo nivel (motores, PWM, encoders)  
-  **Jetson Nano** → Procesamiento, control y generación de trayectorias  
-  **Estructura impresa en 3D** → Diseño modular y replicable  

---

## 📦 Contenido del repositorio

- 📁 `/stl` → Archivos STL para impresión 3D  
- 📁 `/arduino` → Código para control de motores  
- 📁 `/jetson` → Scripts en Python para control y trayectorias  
- 📁 `/videos` → Ejecución de trayectorias (lineal, circular, sinusoidal)  

---

## Estado del proyecto

⚠️ Este prototipo se encuentra en desarrollo.

El diseño, código y estrategias de control están en **constante evolución**.  
Se recomienda revisar este repositorio periódicamente para acceder a:

-  Mejoras  
-  Correcciones  
-  Nuevas funcionalidades experimentales  

---

##  Aplicaciones

- Seguimiento de trayectoria  
- Control de robots diferenciales  
- Sistemas embebidos educativos  
- Experimentación en robótica móvil  

---

##  Contribuciones

Las contribuciones son bienvenidas.  
Puedes aportar mediante:

- Mejoras en código  
- Optimización de diseño  
- Nuevas estrategias de control  
- Documentación  

---

## Como Citar el trabajo

> Por favor, cite la publicación original al utilizar o adaptar cualquier parte de este proyecto.

MDPI and ACS Style

Márquez-Sánchez, C.; Sandoval-Gutiérrez, J.; Martínez-Vázquez, D.L. Construction of an Educational Prototype of a Differential Wheeled Mobile Robot. Hardware 2026, 4, 2. https://doi.org/10.3390/hardware4010002

AMA Style

Márquez-Sánchez C, Sandoval-Gutiérrez J, Martínez-Vázquez DL. Construction of an Educational Prototype of a Differential Wheeled Mobile Robot. Hardware. 2026; 4(1):2. https://doi.org/10.3390/hardware4010002

Chicago/Turabian Style

Márquez-Sánchez, Celso, Jacobo Sandoval-Gutiérrez, and Daniel Librado Martínez-Vázquez. 2026. "Construction of an Educational Prototype of a Differential Wheeled Mobile Robot" Hardware 4, no. 1: 2. https://doi.org/10.3390/hardware4010002

APA Style

Márquez-Sánchez, C., Sandoval-Gutiérrez, J., & Martínez-Vázquez, D. L. (2026). Construction of an Educational Prototype of a Differential Wheeled Mobile Robot. Hardware, 4(1), 2. https://doi.org/10.3390/hardware4010002


---

## 📬 Contacto

**Celso Márquez-Sánchez**  
Universidad Autónoma Metropolitana – Lerma, México  
📧 c.marquez@correo.ler.uam.mx  

---

<p align="center">
  Si este proyecto te es útil, considera darle una estrella en GitHub ⭐
</p>
